...
use hedl_stream::{LineReader, NodeEvent, StreamError, StreamingParser, Value};
use std::io::Cursor;
...
#[test]
fn test_list_literal_values() {
    let input = r#"
%VERSION: 1.1
%STRUCT: User: [id, roles, flags, meta]
---
users:@User
 | alice, (admin, editor, viewer), (true, false, true), (a, b, c)
 | bob, (viewer), (false), ()
"#;
    ...
    if let Value::List(roles) = &nodes[0].fields[1] {
        assert_eq!(roles.len(), 3);
        assert!(matches!(roles[0], Value::String(ref s) if &**s == "admin"));
        assert!(matches!(roles[1], Value::String(ref s) if &**s == "editor"));
        assert!(matches!(roles[2], Value::String(ref s) if &**s == "viewer"));
    } else {
        panic!("Expected List for roles field");
    }

    if let Value::List(flags) = &nodes[0].fields[2] {
        assert_eq!(flags.len(), 3);
        assert!(matches!(flags[0], Value::Bool(true)));
        assert!(matches!(flags[1], Value::Bool(false)));
        assert!(matches!(flags[2], Value::Bool(true)));
    } else {
        panic!("Expected List for flags field");
    }

    // Bob's list fields
    assert_eq!(nodes[1].id, "bob");
    if let Value::List(roles) = &nodes[1].fields[1] {
        assert_eq!(roles.len(), 1);
        assert!(matches!(roles[0], Value::String(ref s) if &**s == "viewer"));
    } else {
        panic!("Expected List for roles field");
    }

    if let Value::List(meta) = &nodes[1].fields[3] {
        assert_eq!(meta.len(), 0, "Empty list should have 0 elements");
    } else {
        panic!("Expected List for meta field");
    }
}

#[test]
fn test_mode_and_prompt_directives_recognized() {
    let input = r#"
%V:2.0
%NULL:~
%QUOTE:"
%MODE: strict
%PROMPT: This is a test schema for user data
%STRUCT: User: [id, name, status]
---
users:@User
 | alice, Alice Smith, active
"#;
    let parser = StreamingParser::new(Cursor::new(input)).unwrap();
    assert_eq!(parser.header().unwrap().version, (2, 0));
}
