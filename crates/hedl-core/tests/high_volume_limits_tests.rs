...
fn test_deeply_nested_hierarchy() {
    ...
    // Define schemas for each level (in header, before ---)
    for i in 0..depth {
        doc.push_str(&format!("%S:Level{i}:[id,name]\n"));
    }

    // Define NEST relationships (in header, before ---)
    for i in 0..depth - 1 {
        doc.push_str(&format!("%N:Level{}>Level{}\n", i, i + 1));
    }
...
fn test_nested_hierarchy_at_limit() {
    ...
    for i in 0..depth {
        doc.push_str(&format!("%S:L{i}:[id]\n"));
    }

    for i in 0..depth - 1 {
        doc.push_str(&format!("%N:L{}>L{}\n", i, i + 1));
    }
...
fn test_nest_depth_limit_enforced() {
    ...
    // Define schemas for each level (in header, before ---)
    for i in 0..depth {
        doc.push_str(&format!("%S:Level{i}:[id]\n"));
    }
    ...
    assert!(
        result.is_err(),
        "Expected parsing to fail due to depth limit"
    );
...
fn test_nest_depth_within_limit_succeeds() {
    ...
    assert!(
        result.is_ok(),
        "Expected parsing to succeed within depth limit"
    );
...
fn test_nest_depth_limit_minimal() {
    ...
    let mut doc = String::from("%V:2.0\n%NULL:~\n%QUOTE:\"\n");
    doc.push_str("%S:Parent:[id]\n");
    doc.push_str("%S:Child:[id]\n");
    doc.push_str("%N:Parent>Child\n");
    doc.push_str("---\ndata:@Parent\n");
    ...
    assert!(
        result.is_err(),
        "Expected parsing to fail with depth limit of 1"
    );
    assert!(matches!(result.unwrap_err().kind, HedlErrorKind::Security));
}
...
fn test_max_object_keys_limit_enforced() {
    ...
    assert!(
        result.is_err(),
        "Expected parsing to fail due to max_object_keys limit"
    );
    let err = result.unwrap_err();
    assert!(
        matches!(err.kind, HedlErrorKind::Security),
...
fn test_max_total_keys_limit_enforced() {
    ...
    assert!(
        result.is_err(),
        "Expected parsing to fail due to max_total_keys limit"
    );
...
fn test_max_total_keys_nested_objects() {
    ...
    assert!(
        result.is_err(),
        "Expected parsing to fail due to total keys limit across nesting"
    );
    assert!(matches!(result.unwrap_err().kind, HedlErrorKind::Security));
}
...
fn test_max_total_keys_overflow_protection() {
    ...
    assert!(
        result.is_ok(),
        "Expected parsing to succeed with overflow protection"
    );
}
...
fn test_max_total_keys_excludes_matrix_schemas() {
    ...
    let mut doc = String::from("%V:2.0\n%NULL:~\n%QUOTE:\"\n");
    doc.push_str("%S:Record:[id,name,age,city,country]\n");
    doc.push_str("---\n");
    ...
    assert!(
        result.is_ok(),
        "Matrix schema columns should not count toward max_total_keys"
    );
}
...
fn test_max_total_keys_includes_block_strings() {
    ...
    assert!(
        result.is_ok(),
        "Block string keys should count toward total, but 5 is at limit"
    );
    ...
    assert!(
        result.is_err(),
        "Should fail when exceeding limit with extra key"
    );
}
