...
// Re-export public function from lookup module
...
impl<'a> InferenceContext<'a> {
    ...
    pub fn for_key_value(aliases: &'a BTreeMap<String, String>) -> Self {
        Self {
            ...
            null_char: '~',  // Default null character
            expected_type: None,
            column_types: None,
            strict_types: false,
...
pub fn infer_value(s: &str, ctx: &InferenceContext<'_>, line_num: usize) -> HedlResult<Value> {
    ...
    // Check for null literal using the configured null_char (from %NULL directive)
    // This handles custom null characters like '-' if %NULL:- was specified
    ...
    // This lookup typically handles 30-50% of values in real HEDL documents
        ...
        Some(b'(') => {
            ...
            match parse_list_literal(trimmed, 0) {
                Ok((lex_value, _consumed)) => {
                    // Convert lex_inference::Value to value::Value
    ...
    // Use Box::from() directly to avoid intermediate String allocation
...
fn infer_ditto(ctx: &InferenceContext<'_>, line_num: usize) -> HedlResult<Value> {
    ...
    // v2.0 does not allow ditto operator
    if ctx.version >= (2, 0) {
