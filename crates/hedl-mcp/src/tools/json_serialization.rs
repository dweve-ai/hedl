...
pub fn node_to_json(node: &Node, schema: &[String], include_children: bool) -> JsonValue {
    ...
    for (i, value) in node.fields.iter().enumerate() {
        if let Some(field_name) = schema.get(i) {
            fields.insert(field_name.as_str().to_string(), value_to_json(value));
        }
    }
...
mod tests {
    ...
    fn test_value_to_json_list() {
        ...
        assert_eq!(arr[1], "editor");
        assert_eq!(arr[2], "viewer");
    }

    #[test]
    fn test_value_to_json_empty_list() {
    ...
    #[test]
    fn test_value_to_json_mixed_list() {
        let list = Value::List(Box::new(vec![
            Value::Int(42),
