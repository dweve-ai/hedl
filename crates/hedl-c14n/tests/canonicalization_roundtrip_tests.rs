...
#[test]
fn test_version_output_v11() {
    // v1.1 uses long format
    let doc = Document::new((1, 1));
    let output = canonicalize(&doc).unwrap();
    assert!(output.starts_with("%VERSION: 1.1"));
}

#[test]
fn test_version_output_compact() {
    // Compact format with %NULL:~ and %QUOTE:"
    let doc = Document::new((1, 2));
    let output = canonicalize(&doc).unwrap();
    assert!(output.starts_with("%V:1.2"));
    assert!(output.contains("%NULL:~"));
    assert!(output.contains("%QUOTE:\""));
}

#[test]
fn test_version_output_v15_compact() {
    // Compact format with %NULL:~ and %QUOTE:"
    let doc = Document::new((1, 5));
    let output = canonicalize(&doc).unwrap();
    assert!(output.starts_with("%V:1.5"));
    assert!(output.contains("%NULL:~"));
    assert!(output.contains("%QUOTE:\""));
}
...
fn test_alias_order() {
    ...
    let output = canonicalize(&doc).unwrap();
    // Uses %A: prefix
    let apple_pos = output.find("%A:%apple").unwrap();
    let mango_pos = output.find("%A:%mango").unwrap();
    let zebra_pos = output.find("%A:%zebra").unwrap();
...
fn test_struct_order() {
    ...
    // Uses %S: prefix
    let apple_pos = output.find("%S:Apple:").unwrap();
    let zebra_pos = output.find("%S:Zebra:").unwrap();
...
fn test_ditto_optimization() {
    ...
    // Second row should repeat the value explicitly (ditto disabled by default)
    assert!(output.contains("|i2,fruit"));
}
...
fn test_ditto_deep_equality() {
    ...
    // Boolean values should be repeated explicitly (ditto disabled by default)
    assert!(output.contains("|i2,true"));
}
...
fn test_ditto_disabled() {
    ...
    // Values are always explicit (ditto removed in new format)
...
fn test_empty_last_column_with_ditto() {
    ...
    assert!(output.contains("|i1,data,\"\""));
    // Second row: values are repeated explicitly (ditto disabled by default)
    assert!(output.contains("|i2,data,\"\""));
}
...
"#;
    ...
    // Row 2: explicit value (ditto removed in new format)
    assert!(
        canonical.contains("|i2,value,\"\""),
        "Non-empty middle, explicit empty last"
    );
...
// =============================================================================
// Inline Child List Tests
// =============================================================================

#[test]
fn test_inline_children_format_small() {
    // Create document with nested children (<= 5)
    let mut doc = Document::new((2, 0));
    doc.structs.insert(
        "Product".to_string(),
    ...
    doc.structs.insert(
        "Review".to_string(),
        vec!["id".to_string(), "rating".to_string(), "text".to_string()],
    );
    doc.nests
        .insert("Product".to_string(), vec!["Review".to_string()]);

    let mut product_list = MatrixList {
        type_name: "Product".to_string(),
        schema: vec!["id".to_string(), "name".to_string(), "price".to_string()],
        rows: Vec::new(),
        count_hint: None,
    };

    // Create product with 3 reviews (should use inline format)
    let mut product1 = Node::new(
        "Product",
        "prod-001",
        vec![
            Value::String("prod-001".to_string().into()),
            Value::String("Laptop".to_string().into()),
            Value::Float(999.99),
        ],
    );
    ...
    product1.add_child(
        ...
        Node::new(
            ...
            "rev-001",
            vec![
                Value::String("rev-001".to_string().into()),
                Value::Int(5),
    ...
    product1.add_child(
        ...
        Node::new(
            ...
            "rev-002",
            vec![
                Value::String("rev-002".to_string().into()),
                Value::Int(4),
                Value::String("Good".to_string().into()),
            ],
        ),
    );
    ...
    product1.add_child(
        ...
        Node::new(
            ...
            "rev-003",
            vec![
                Value::String("rev-003".to_string().into()),
                Value::Int(5),
                Value::String("Excellent".to_string().into()),
            ],
        ),
    );

    product_list.rows.push(product1);
    doc.root
        .insert("products".to_string(), Item::List(product_list));

    let output = canonicalize(&doc).unwrap();
    println!("Output:\n{}", output);

    // Check for inline format:@Review#3:|...
    assert!(
        output.contains("@Review#3:"),
        "Should contain inline child declaration"
    );
    assert!(
        output.contains("@Review#3:|rev-001,5,Great|rev-002,4,Good|rev-003,5,Excellent"),
        "Should contain inline children on same line"
    );
}

#[test]
fn test_inline_children_format_expanded() {
    // Create document with nested children (> 5)
    let mut doc = Document::new((2, 0));
    doc.structs.insert(
        "Product".to_string(),
    ...
    // Create product with 7 reviews (should use expanded format)
    let mut product1 = Node::new(
        "Product",
        "prod-001",
        vec![
    ...
    for i in 1..=7 {
        product1.add_child(
            ...
            Node::new(
                ...
                format!("rev-{:03}", i),
                vec![
                    Value::String(format!("rev-{:03}", i).into()),
                    Value::Int(i as i64),
                ],
            ),
        );
    ...
    assert!(
        output.contains("@Review#7:"),
        "Should contain child type declaration"
    );
    // Should NOT have all children on one line (expanded format)
    assert!(
        output.contains(" |rev-001,1"),
        "Should have children on separate lines"
    );
    assert!(
        output.contains(" |rev-007,7"),
        "Should have all children on separate lines"
    );
}

#[test]
fn test_inline_children_roundtrip() {
    // Test that inline children can be parsed and re-canonicalized correctly
    let hedl_input = r#"%V:1.2
...
products:@Product
 |prod-001,Laptop,999.99
  @Review#3:|rev-001,5,Great|rev-002,4,Good|rev-003,5,Excellent
"#;

    // Parse the input
    let doc = parse(hedl_input.as_bytes()).expect("Failed to parse");

    // Canonicalize it
    let output = canonicalize(&doc).expect("Failed to canonicalize");

    println!("Output:\n{}", output);

    // Parse the output again
    let doc2 = parse(output.as_bytes()).expect("Failed to parse output");

    // Verify data integrity
    let list1 = doc.root.get("products").unwrap().as_list().unwrap();
    let list2 = doc2.root.get("products").unwrap().as_list().unwrap();

    assert_eq!(list1.rows.len(), list2.rows.len(), "Row count mismatch");

    let row1 = &list1.rows[0];
    let row2 = &list2.rows[0];

    assert_eq!(row1.fields.len(), row2.fields.len(), "Field count mismatch");

    let children1 = row1.children().unwrap();
    let children2 = row2.children().unwrap();

    let reviews1 = children1.get("Review").unwrap();
    let reviews2 = children2.get("Review").unwrap();

    assert_eq!(reviews1.len(), reviews2.len(), "Review count mismatch");
    assert_eq!(reviews1.len(), 3, "Should have 3 reviews");

    // Verify the output still uses inline format
    assert!(
        output.contains("@Review#3:|rev-001,5,Great|rev-002,4,Good|rev-003,5,Excellent"),
        "Output should preserve inline format"
    );
}
