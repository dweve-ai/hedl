...
#[test]
fn test_array_root_is_valid() {
    // JSON arrays are valid JSON documents per the spec
    let result = json_to_hedl("[1, 2, 3]");
    assert!(result.is_ok(), "Array root should be valid JSON");

    // Object arrays become matrix lists
    let result = json_to_hedl(r#"[{"id": "a"}, {"id": "b"}]"#);
    assert!(result.is_ok(), "Object array root should be valid");
}

#[test]
fn test_primitive_root_error() {
    // Primitives (not objects or arrays) are invalid roots
    let result = json_to_hedl("42");
    assert!(result.is_err(), "Number root should be invalid");

    let result = json_to_hedl("\"hello\"");
    assert!(result.is_err(), "String root should be invalid");

    let result = json_to_hedl("true");
    assert!(result.is_err(), "Bool root should be invalid");

    let result = json_to_hedl("null");
    assert!(result.is_err(), "Null root should be invalid");
}
...
fn test_empty_document() {
    ...
    // json_to_hedl creates new documents with v2.0 default
...
fn test_tensors_roundtrip_json() {
    ...
    // In HEDL v1.1, empty arrays in JSON are converted to Value::List (empty inline list).
    // The original tensor_empty value becomes an empty inline list ().
    if let Some(Item::Scalar(Value::List(items))) = restored.root.get("tensor_empty") {
        assert!(items.is_empty(), "Expected empty list for tensor_empty");
    } else {
        panic!("Expected tensor_empty as empty Value::List");
    }
}
...
fn test_comprehensive_roundtrip_json() {
    ...
    // (children roundtrip requires matching nesting logic)
    let config = ToJsonConfig {
        include_metadata: false,
        flatten_lists: true,
...
fn test_empty_roundtrip_json() {
    ...
    // Verify empty document - json_to_hedl creates new documents with default version
    assert_eq!(restored.version, (2, 0));
    assert!(restored.root.is_empty());
